...
CASE WHEN b.memid =0
THEN guestcost * b.slots
ELSE membercost * b.slots
END AS cost
FROM Bookings AS b
INNER JOIN Facilities AS f ON b.facid = f.facid
AND starttime LIKE '2012-09-14%'
AND ((b.memid =0 AND guestcost * b.slots >30)
OR (b.memid <>0 AND membercost * b.slots >30))
INNER JOIN Members AS m ON b.memid = m.memid
ORDER BY cost DESC
...
SELECT *
FROM (SELECT sub.facility as facility, SUM(sub.cost) AS revenue
      FROM (
          SELECT Facilities.name as facility, 
          CASE WHEN Bookings.memid = 0 THEN Bookings.slots * guestcost
          ELSE Bookings.slots * membercost END AS cost
          FROM Bookings
          INNER JOIN Facilities
          ON Bookings.facid = Facilities.facid ) AS sub
